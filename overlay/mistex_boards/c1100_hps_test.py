#!/usr/bin/env python3
#
# C1100 HPS transport test: the PCIe endpoint of c1100_pcie_video.py plus the
# hps_pcie bridge (rtl/hps_pcie) driving a real hps_io.sv with a test config
# string, in its own 50 MHz "core" clock domain.  This is the bitstream
# Main_MiSTeX's PCIe backend (host/main_mistex_pcie) talks to: it reads the
# config string, shows the menu, sends joystick and status words, all of which
# can be read back here through CSRs -- the host transport of the "three
# things every core needs" (docs/retro-cores.md), verified without a core,
# a memory or a video sink.
#
# Build (from a MiSTeX-ports checkout with the overlay installed):
#   venv/bin/python mistex_boards/c1100_hps_test.py --build
#
...
class _CRG(LiteXModule):
    ...
    def __init__(self, platform, sys_clk_freq, core_clk_freq=50e6):
        self.rst     = Signal()
...
class HPSTestSoC(SoCMini):
    def __init__(self, platform, speed="gen3", nlanes=4, sys_clk_freq=125e6):
        SoCMini.__init__(self, platform, sys_clk_freq, ident=f"C1100 HPS transport test x{nlanes} {speed}")
...
def build(nlanes=4, speed="gen3", do_build=False, build_dir="build/c1100_hps_test"):
