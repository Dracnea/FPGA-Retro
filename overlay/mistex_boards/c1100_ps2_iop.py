...
#
# Build (from a MiSTeX-ports checkout with this overlay installed):
#   venv/bin/python mistex_boards/c1100_ps2_iop.py --build
...
PORTS_ROOT = normpath(join(dirname(abspath(__file__)), ".."))   # the MiSTeX-ports checkout


# IOP sources --------------------------------------------------------------------------------------

def add_iop_sources(platform, root=PORTS_ROOT):
    """The same file list as cores/PS2/fit/fit_iop.tcl and sim/run_sim.sh.
    ...
    """
    up   = join(root, "cores", "PSX", "upstream", "rtl")
    ovl  = root
    core = join(root, "cores", "PS2")
    platform.add_source(join(ovl, "rtl", "altera_compat", "altera_mf_components.vhd"),
                        language="vhdl", library="altera_mf")
    compat_v = [join(ovl, "rtl", "altera_compat", f) for f in ("altsyncram.v", "altdpram.v")]
    for f in compat_v:
    ...
    vhdl = [
        ...
        join(ovl, "cores", "PSX", "rtl", "SyncRamDual.vhd"),
        join(ovl, "cores", "PSX", "rtl", "SyncRamDualNotPow2.vhd"),
