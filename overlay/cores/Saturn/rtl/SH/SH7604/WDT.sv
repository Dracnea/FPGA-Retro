...
// made reg; no functional change. Upstream: upstream/rtl/SH/SH7604/WDT.sv
module SH7604_WDT
...
(
	...
	always @(posedge CLK or negedge RST_N) begin
		if (!RST_N) begin
			// (synopsys translate_off: removed for Vivado, which took the pair as unmatched and dropped the rest of the file)
			WDTOVF_N <= 1;
			WRES <= 0;
			// (synopsys translate_on)n
		end
		else begin
			if (!RES_N) begin
				WDTOVF_N <= 1;
