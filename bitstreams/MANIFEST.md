...
All C1100 images from 2026-09-07 21:00 on declare BAR0 **64-bit prefetchable**; the earlier ones had a 32-bit BAR that this host places in a window its firmware never routed, so every register read returned `0xffffffff` (`docs/c1100-pcie-transport.md`). `c1100_hps_test.bit` (md5 `23a727f5…`) is the one image not yet rebuilt with it.
